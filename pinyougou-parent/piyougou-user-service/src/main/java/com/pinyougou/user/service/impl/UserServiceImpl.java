...
public class UserServiceImpl implements UserService {
	...
	public void add(TbUser user) {
		user.setCreated(new Date());//会员注册时间
		user.setUpdated(new Date());//会员更新时间
		user.setSourceType("1");//通过pc端注册
   ...
   private RedisTemplate redisTemplate;
	...
	public void createSmsCode(final String phone) {
		...
		//将短信发送给activeMq
		jmsTempalte.send(smsDestination, new MessageCreator() {
			@Override
	...
	public boolean checkSmsCode(String phone, String code) {
		...
		//判断缓存的验证码和输入的验证码
		if (smsCode==null){//生成验证码为null
			return false;
		}
		if (!smsCode.equals(code)){//生成验证码与输入验证码不一致
			return false;
		}
		return true;
	}
